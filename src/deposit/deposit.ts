...
export const deposit = async (depositAmount: string, starkKey: number, positionId: number, signature: string) => {
    try {
        ...
        console.log(BigInt(starkKey))

        BigInt

        const tx = await contract.callStatic.deposit(depositAmount, starkKey, positionId, signature)
